...
Node *node_create(uint8_t symbol, uint64_t frequency) {
    ...
    if (n) {
        ...
        // }
        n->left = NULL;
        n->right = NULL; // make sure left and right pointers are NULL
        n->symbol = symbol; // sets the node’s symbol as symbol
        n->frequency = frequency; // sets the node’s frequency as frequency
    }
    return n;
...
Node *node_join(Node *left, Node *right) {
    ...
    uint64_t parent_frequency
        = left_frequency
          + right_frequency; // parent node's frequency is the sum of its left child's frequency and its right child's frequency
    ...
    Node *parent_node = node_create('$',
        parent_frequency); // creates parent node with symbol '$' and frequency: parent_frequency
